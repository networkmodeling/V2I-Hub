//============================================================================
// Name        : HRIPredtcedSpatPlugin.cpp
...
namespace HRIPredictedSpatPlugin
{
...
class HRIPredictedSpatPlugin: public PluginClient
{
public:
	HRIPredictedSpatPlugin(std::string);
	virtual ~HRIPredictedSpatPlugin();
	int Main();
 ...
 */
HRIPredictedSpatPlugin::HRIPredictedSpatPlugin(string name) : PluginClient(name)
{
	AddMessageFilter<BsmMessage>(this, &HRIPredictedSpatPlugin::HandleBSMMessage);
	SubscribeToMessages();
...
HRIPredictedSpatPlugin::~HRIPredictedSpatPlugin()
{
	// if (_portName == "")
	// {
	// 	if (!wdtPresent())
...
void HRIPredictedSpatPlugin::OnConfigChanged(const char *key, const char *value)
{
...
void HRIPredictedSpatPlugin::OnStateChange(IvpPluginState state)
{
...
void HRIPredictedSpatPlugin::UpdateConfigSettings()
{
...
void HRIPredictedSpatPlugin::HandleBSMMessage(BsmMessage &msg, routeable_message &routeableMsg)
{
	if(!_isReceivingBsms)
	{
 ...
 */
bool HRIPredictedSpatPlugin::GetPinState(int pinNumber)
{
	// int i;
	// if (_portName == "")
	// {
 ...
 */
void HRIPredictedSpatPlugin::MonitorRailSignal()
{
	while(!_stopThreads)
	{
...
uint16_t HRIPredictedSpatPlugin::GetCrc16(uint16_t crc, uint8_t *data, uint16_t length)
{
	if (length)
	{
		while (length--)
...
uint32_t HRIPredictedSpatPlugin::GetCrc32(uint32_t crc, uint8_t *data, uint16_t length)
{
//	uint32_t index;
	if (length)
	{
...
void HRIPredictedSpatPlugin::UpdateTimestamp(message_document &md)
{
	struct timeval tv;
...
void HRIPredictedSpatPlugin::UpdateMovementState(message_document &md)
{
	...
	for(std::pair<int, std::string> newState : _laneMapping)
	{
		...
		if (_minEndTime >= 0 and _maxEndTime >= 0)
		{
			timing.append_child("minEndTime").append_child(pugi::node_pcdata).set_value(std::to_string(_minEndTime).c_str());
			timing.append_child("maxEndTime").append_child(pugi::node_pcdata).set_value(std::to_string(_maxEndTime).c_str());
		}
		else
		{
...
int HRIPredictedSpatPlugin::Main()
{
...
//	return (EXIT_SUCCESS);
	...
	std::thread trainWatch(&HRIPredictedSpatPlugin::MonitorRailSignal, this);
	// std::thread serialPortReader(&HRIPredictedSpatPlugin::SerialPortReader, this);

	usleep(2000000); //wait for thread to spin up

	HRI_Predicted_SPaT_Plugin_Agent hri_predicted_spat_plugin_agent;
	if (_useSimulatedPreemptionSignal)
	{
		hri_predicted_spat_plugin_agent.HRI_Predicted_SPaT_Plugin_Agent_Initialize();
	}
	usleep(7000);
	...
	while (!IsPluginState(IvpPluginState_error))
	{
		...
		if (_useSimulatedPreemptionSignal)
		{
			hri_predicted_spat_plugin_agent.HRI_Predicted_SPaT_Plugin_Agent_receive_from_digital();
			if (hri_predicted_spat_plugin_agent.get_preemption_signal_status())
...
{
	return run_plugin<HRIPredictedSpatPlugin::HRIPredictedSpatPlugin>("HRIPredictedSpatPlugin", argc, argv);
}
