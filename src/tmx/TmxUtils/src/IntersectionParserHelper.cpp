...
WGS84Point GetPointFromNode(WGS84Point &referencePoint, long &latval, long &longval, double& totalXOffset, double& totalYOffset,
		long elevationval = 0, double totalElevationOffset = 0.0) {
	...
	double scaleOffset = 0.01; //x and y offsets are in centimeters
	...
	elevationOffset = elevationval * 0.1; //elevation offset is in decimeters
	totalElevationOffset = totalElevationOffset * 0.1; //elevation offset is in decimeters
	...
	nodeOffsetPoint.Elevation = referencePoint.Elevation + totalElevationOffset;
...
std::list<LaneNode> ComputeLaneNodes(WGS84Point refPoint, int laneId, GenericLane **lanes, int laneCount,
	...
	double totalElevationOffset = 0.0;
	...
	for (int i = 0; i < laneCount; i++)
	{
		...
		if (lane->nodeList.present == DEFINED_NODE)
		{
			...
			for (int j = 0; j < lane->nodeList.choice.nodes.list.count; j++)
			{
				...
				if (node->attributes && node->attributes->dElevation)
					elevation = *(node->attributes->dElevation);

				LaneNode laneNode;
				laneNode.Point = tmx::utils::GetPointFromNode(refPoint, x, y, totalXOffset, totalYOffset, elevation, totalElevationOffset);
				totalElevationOffset += elevation;
			...
			for (int j = 0; j < laneCount; j++)
			{
				...
				for (int k = 0; k < referenceLane->nodeList.choice.nodes.list.count; k++)
				{
					Node *node = referenceLane->nodeList.choice.nodes.list.array[k];
						...
						laneNode.Point = tmx::utils::GetPointFromNode(refPoint, x, y, totalComputedXOffset, totalComputedYOffset, elevation, totalElevationOffset);
						totalElevationOffset += elevation;
						laneNodes.push_back(laneNode);
				}
			}
