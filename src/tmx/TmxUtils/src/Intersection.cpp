...
bool Intersection::IsSignalForGroupRedLight(tmx::messages::SpatMessage &msg, int signalGroup) {
	...
	{
		for (int j = 0; j < interX->list.array[i]->states.list.count; j++)
		{
...
int Intersection::GetSignalGroupForLocation(double lat, double lon)
{
	if(!_isSpatLoaded || !_isMapLoaded)
		return -1;
...
//	PLOG(logDEBUG) << "GetSignalForLocation: IsInLane: " << r.IsInLane << ", LaneNumber: " << r.LaneNumber << ", LaneSegment: " << r.LaneSegment;
	...
	return mapSupp.GetSignalGroupForVehicleLane(r.LaneNumber, Map);
}
...
MapMatchResult Intersection::GetApproachLaneForLocation(double lat, double lon, double heading)
{

	MapMatchResult failedResult;
	failedResult.PerpDistanceMeters = 0;
	failedResult.StopDistanceMeters = 0;
	failedResult.IsInLane = false;
	failedResult.LaneSegment = 0;
	failedResult.IsEgress = false;
	failedResult.LaneNumber = -2;
	if(!_isMapLoaded)
	{
		return failedResult;
	}
	...
	if (r.IsInLane && r.LaneNumber > 0)
	{
		MapLane lane = this->GetMapLane(MapMessage, r.LaneNumber);
		double angle = GeoVector::AngleBetweenPathsInDegrees(location, heading, lane.Nodes.back().Point, lane.Nodes.front().Point);
		if (angle > 45)
		{
			failedResult.LaneNumber = -1;
			return failedResult;
		}
	}
	return r;
